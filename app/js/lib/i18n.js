...
angular.module('Qemy.i18n', [])
    ...
    .directive('qemyI18n', ['_', function(_) {
        return {
            restrict: 'EA',
            compile: function(element) {
                var params = element.children('qemy-i18n-param:not([name]), [qemy-i18n-param=""]:not([name])').map(function(index, param) {
                    return param.outerHTML;
                }).toArray();
                element.children('qemy-i18n-param[name], [qemy-i18n-param]:not([qemy-i18n-param=""]), [qemy-i18n-param][name]').each(function(i, param) {
                    params[angular.element(param).attr('qemy-i18n-param') || angular.element(param).attr('name')] = param.outerHTML;
                });
                element.children('qemy-i18n-param').remove();
                var formats = element.attr("qemy-i18n") || element.attr("msgid") ? element : element.children('qemy-i18n-format, [qemy-i18n-format]');
                formats.each(function(index, element) {
                    var format = angular.element(element);
                    var msgid = format.attr("qemy-i18n") || format.attr("msgid") || format.attr("qemy-i18n-format") || format.html().replace(/\s+/g, ' ').trim();
